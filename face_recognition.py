...
import math
# face_recognition.py shape_predictor_68_face_landmarks.dat dlib_face_recognition_resnet_model_v1.dat videos/bring-reading-rainbow-back-for-every-child-everywh/testimages
if len(sys.argv) != 4:
    print(
...
reference = face_list[0]
face_differences = []
for i in range(1, len(face_list)):
    diff_vector = []
    for j in range(len(reference)):
        diff_vector.append(math.fabs(reference[j]-face_list[i][j]))
    face_differences.append(diff_vector)
diff_list = []
for item in face_differences:
    a = 0
    for number in item:
        a += number
    diff_list.append(a)
